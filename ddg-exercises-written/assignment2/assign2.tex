...
the area of the spherical polygon approaches zero, because the normals all meet at a single point on the Gauss map.\\
...
$\chi = (2 - 2g) = |V| - |E| + |F|$ is a topological invariant that does not change unless we change the genus $g$ of the surface,
or change the number of mesh elements ($|V|, |E|, |F|$) in a way that changes $\chi$. 
